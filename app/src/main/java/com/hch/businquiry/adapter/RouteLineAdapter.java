    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        switch (mtype) {
            case  TRANSIT_ROUTE:
                ...
                int time1 = routeLines.get(position).getDuration();
                if ( time1 / 3600 == 0 ) {
                    holder.lightNum.setText( "大约需要：" + time1 / 60 + "分钟" );
                } else {
                    holder.lightNum.setText( "大约需要：" + time1 / 3600 + "小时" + (time1 % 3600) / 60 + "分钟" );
                }
                holder.dis.setText("距离大约是：" + routeLines.get(position).getDistance() + "米");
                TransitRouteLine rtl = (TransitRouteLine)routeLines.get(position);
                TaxiInfo taxiInfo = rtl.getTaxitInfo();
                TransitRouteLine rl =  (TransitRouteLine)routeLines.get(0);
                List<TransitRouteLine.TransitStep> ts = rl.getAllStep();
                String str = "";
                for (TransitRouteLine.TransitStep tt : ts){
                    str+=tt.getInstructions()+"\n";
                }
                holder.way.setText("坐车方法："+"\n"+  str);
                break;
            case WALKING_ROUTE:
            case BIKING_ROUTE:
                holder.name.setText("路线" + (position + 1));
